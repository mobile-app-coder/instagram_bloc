...
import 'package:instagram_bloc_cubit/services/utils_service.dart';
...
class DBService {
  ...
  static Future<List<Member>> searchMembers(String keyword) async {
    ...
    List<Member> following = await followings();
    ...
    for (var result in querySnapshot.docs) {
      ...
      if (following.any((element) => element.uid == newMember.uid)) {
        newMember.followed = true;
      }
  ...
  static Future<List<Member>> followings() async {
    List<Member> members = [];
    String uid = AuthService.currentUserid();
